...
def reduce_dimension(docs, reducer_type, model_config, tune):
    print(
        f'\nbegin reducing {reducer_type}, tune: {tune}, shape before: {docs.shape}')

    if reducer_type == 'lsa':
        if tune:
            reducer_model = TruncatedSVD(
                n_components=model_config.lsa__n_components_tune)
            doc_reduced = reducer_model.fit_transform(docs)
            print('plotting lsa components:',
                  model_config.lsa__n_components_tune)

            fig = plt.figure(figsize=(10, 5))
            plt.plot(range(model_config.lsa__n_components_tune),
                     np.cumsum(reducer_model.explained_variance_ratio_))
            ...
            reducer_model = TruncatedSVD(
                n_components=model_config.lsa__n_components)
    ...
    print(f'\n  after reducing shape {doc_reduced.shape}')
